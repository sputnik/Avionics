  ...
  if (SetupRun) {
    ...
    while (current_status == 0) {
      ...
      if (curr_time > (old_time + 40)) {
        sprintf(SD_data, "The Current Data Values  (On Pad) are:\tHeight: %06d\tVelocity: %06d\tAcceleration: %06d\tPressure: %06d\tThe time is: %02d:%02d:%02d\tThe date is: %02d/%02d/%04d",(int)AvgHeight, (int)(AvgVelocity * 1000), (int)(VerticalAccelBNO*1000), (int)HeightPress, rtc_time[0], rtc_time[1], rtc_time[2], rtc_time[3], rtc_time[4], rtc_time[5]);
        write_to_SD();
        old_time = millis();
      }
    }
    ...
    while (current_status == 1) {
      ...
      curr_time = millis();
      if (curr_time > (old_time + 40)) {
        sprintf(SD_data, "The Current Data Values  (Launching) are:\tHeight: %06d\tVelocity: %06d\tAcceleration: %06d\tPressure: %06d\tThe time is: %02d:%02d:%02d\tThe date is: %02d/%02d/%04d",(int)AvgHeight, (int)(AvgVelocity * 1000), (int)(VerticalAccelBNO*1000), (int)HeightPress, rtc_time[0], rtc_time[1], rtc_time[2], rtc_time[3], rtc_time[4], rtc_time[5]);
        write_to_SD();
        old_time = millis();
      }
    }
    ...
    while (current_status == 2) {
      ...
      curr_time = millis();
      if (curr_time > (old_time + 40)) {
        sprintf(SD_data, "The Current Data Values  (Braking) are:\tHeight: %06d\tVelocity: %06d\tAcceleration: %06d\tPressure: %06d\tDirectionPin: %06d\tPWMPin: %06d\tFDBKpin: %06d\tThe time is: %02d:%02d:%02d\tThe date is: %02d/%02d/%04d",(int)AvgHeight, (int)(AvgVelocity * 1000), (int)(VerticalAccelBNO*1000), (int)HeightPress, (int)LAdir, (int)LApwm, (int)LAfdbk, rtc_time[0], rtc_time[1], rtc_time[2], rtc_time[3], rtc_time[4], rtc_time[5]);
        write_to_SD();
        old_time = millis();
      }
    }
