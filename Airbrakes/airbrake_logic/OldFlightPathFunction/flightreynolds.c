...
//# define AREA_BRAKE 0.0032258
//
#define AREA_BRAKE 0.0028432201
...
#define VISCOSITY .0000185 // pascal seconds
#define LENGTH 3.5// this will change
// N2850
...
double drag_from_density_and_velocity(double dense, double vel, double coeffd) //drag equation for rocket
{
	return .5 * dense * vel * vel * coeffd * AREA_ROCKET;
}
double brake_from_density_and_velocity(double dense, double vel, double coeffdbrake) //drag equation for airbrakes
{
	return .5 * dense * vel * vel * coeffdbrake * AREA_BRAKE;
}
...
int main(void)
{
	...
	for (time = 0; time < 10.8; time += TIME_DELTA) {
		...
		cd = something based on reynolds number;

		drag = drag_from_density_and_velocity(density, velocity, cd); //newtons
	...
	for (; velocity > 0; time += TIME_DELTA) {
		...
		reynolds = velocity * density * LENGTH / VISCOSITY;
		cd = something based on reynolds number;
		cdbrake = something else based on reynolds;
		drag = drag_from_density_and_velocity(density, velocity, cd); //newtons
		brake = brake_from_density_and_velocity(density, velocity, cdbrake); //newtons
		k = .5 * density * cd * AREA_ROCKET;
		qsquared = -1 * mass * 9.81 / k;
		hc = ((mass / (2 * k)) * log((qsquared - (velocity*velocity)) / qsquared)) + height;
		fprintf(logfile, "hc is %f\n", hc);
		if ((hc > GOAL_HEIGHT) && (velocity < 343)) {
